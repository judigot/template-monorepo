...
- **Bun** `1.4.2` (pinned in `packageManager`) — the sole package manager.
  Keep the lockfile generated by this pinned version so local and CI installs
...
5. Keep Bun as the sole package manager and regenerate `bun.lock` with the pinned Bun version after dependency changes.
